...
const initializeDatabase = async () => {
    ...
    try {
        ...
        // FIX: Add a simple migration to ensure the `category_ids` column exists in the `brands` table.
        // This prevents errors if the database was created with an older schema.
        const [brandColumns] = await connection.query(
            `SELECT COUNT(*) as count FROM information_schema.columns WHERE table_schema = ? AND table_name = 'brands' AND column_name = 'category_ids'`,
            [dbConfig.database]
        );
        if (brandColumns[0].count === 0) {
            console.log("Cột 'category_ids' không tồn tại trong bảng 'brands'. Đang thêm cột...");
            await connection.query('ALTER TABLE brands ADD COLUMN category_ids JSON');
            console.log("Đã thêm cột 'category_ids' thành công.");
        }
