...
const ProductFormModal: React.FC<ProductFormModalProps> = ({ isOpen, onClose, onSave, product }) => {
  ...
  const getPriceValue = (name: 'price' | 'originalPrice') => {
    const value = formData[name];
    if (activePriceField === name) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input 
                  ...
                  inputMode="numeric"
                  name="price" 
                  value={getPriceValue('price')}
                  onChange={handlePriceChange}
                  onFocus={(e) => setActivePriceField(e.target.name)}
              ...
              <div>
                ...
                <input 
                  ...
                  inputMode="numeric"
                  name="originalPrice"
                  value={getPriceValue('originalPrice')}
                  onChange={handlePriceChange}
                  onFocus={(e) => setActivePriceField(e.target.name)}
                  onBlur={() => setActivePriceField(null)}
                  placeholder="Giá chưa giảm (tùy chọn)"
                  className={formInputClasses}
                />
              </div>
            </div>
